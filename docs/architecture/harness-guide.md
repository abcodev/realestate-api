...
```
src/main/java/com/web/speex/common/
├── logging/
...
```
# 정상 흐름
2026-05-04 14:23:11.482 [http-nio-18083-exec-3] [a1b2c3d4e5f6a7b8] INFO  c.w.k.c.l.ApiLoggingInterceptor - --> GET /speex/api/v1/teams handler=TeamController#getTeams
2026-05-04 14:23:11.534 [http-nio-18083-exec-3] [a1b2c3d4e5f6a7b8] INFO  c.w.k.c.l.ApiLoggingInterceptor - <-- GET /speex/api/v1/teams 200 (52ms)

# 비즈니스 예외 (404)
2026-05-04 14:25:02.110 [http-nio-18083-exec-5] [9f8e7d6c5b4a3210] INFO  c.w.k.c.l.ApiLoggingInterceptor - --> GET /speex/api/v1/teams/999 handler=TeamController#getTeam
...
2026-05-04 14:25:02.143 [http-nio-18083-exec-5] [9f8e7d6c5b4a3210] WARN  c.w.k.c.l.ApiLoggingInterceptor - <-- GET /speex/api/v1/teams/999 404 (33ms) ex=BusinessException: 팀을 찾을 수 없습니다.

# 예상치 못한 예외 (500)
2026-05-04 14:30:45.901 [http-nio-18083-exec-2] [11223344aabbccdd] INFO  c.w.k.c.l.ApiLoggingInterceptor - --> POST /speex/api/v1/games handler=GameController#createGame
...
2026-05-04 14:30:45.999 [http-nio-18083-exec-2] [11223344aabbccdd] ERROR c.w.k.c.l.ApiLoggingInterceptor - <-- POST /speex/api/v1/games 500 (98ms) ex=NullPointerException: null
```
...
grep -rn "import jakarta.persistence\|import org.springframework\|@Entity\|@Table\|@Service\|@Component\|@Repository\|@Transactional" \
  src/main/java/com/web/speex/*/domain/
```
...
```bash
grep -rn "if (\|switch\|} else" src/main/java/com/web/speex/*/application/
```
...
```bash
grep -rn "import com.web.speex.*.infrastructure\|JpaRepository\|JpaEntity" \
  src/main/java/com/web/speex/*/application/
```
...
```bash
grep -rn "import com.web.speex.*.application" src/main/java/com/web/speex/*/infrastructure/
```

### 🔴 P0-5. Service가 DTO를 반환

```bash
grep -rn "import com.web.speex.*.interfaces.dto\|Response \|Request " \
  src/main/java/com/web/speex/*/application/
```
...
```bash
grep -A5 "public.*save" src/main/java/com/web/speex/*/infrastructure/jpa/*Adaptor.java
```
...
```bash
grep -rn "Repository" src/main/java/com/web/speex/*/domain/service/
```
...
```bash
grep -rn "@Setter\|@Data\|public void set" src/main/java/com/web/speex/*/domain/
```
...
ERRORS=0
SRC="src/main/java/com/web/speex"
...
if grep -rn "import com.web.speex.*.infrastructure\|JpaRepository\|JpaEntity" $SRC/*/application/ 2>/dev/null; then
    echo "  ❌ FAIL"; ERRORS=$((ERRORS+1))
else echo "  ✅ PASS"; fi

echo "[P0-4] Infrastructure → Application 역방향 의존..."
if grep -rn "import com.web.speex.*.application" $SRC/*/infrastructure/ 2>/dev/null; then
    echo "  ❌ FAIL"; ERRORS=$((ERRORS+1))
else echo "  ✅ PASS"; fi

echo "[P0-5] Service DTO 반환..."
if grep -rn "import com.web.speex.*.interfaces.dto" $SRC/*/application/ 2>/dev/null; then
    echo "  ❌ FAIL"; ERRORS=$((ERRORS+1))
else echo "  ✅ PASS"; fi
